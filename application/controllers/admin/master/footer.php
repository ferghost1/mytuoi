    </div> <!-- page-body-wrapper in left.php ends -->
  </div> <!--  End container-scroller in head.php -->
  <!-- plugins:js -->
  <script src="assets/template/vendors/js/vendor.bundle.base.js"></script>
  <script src="assets/template/vendors/js/vendor.bundle.addons.js"></script>
  <!-- endinject -->
  <!-- Plugin js for this page-->
  <!-- End plugin js for this page-->
  <!-- inject:js -->
  <script src="assets/template/js/off-canvas.js"></script>
  <script src="assets/template/js/misc.js"></script>
  <!-- endinject -->
  <!-- Custom js for this page-->
  <script src="assets/template/js/dashboard.js"></script>
  <!-- End custom js for this page-->
  <link href="assets/js/dataTable/dataTables.min.css" rel="stylesheet" >
    <script src="assets/js/dataTable/dataTables.min.js">
    </script>
  <!-- Ckeditor-->
    <script src="assets/js/ckeditor/ckeditor.js"></script>
    <script src="assets/js/ckfinder/ckfinder.js"></script>
</body>
  <script defer type="text/javascript">
    $(document).ready( function () {
         $('.datatable').DataTable();
    } );
  </script>
</html>