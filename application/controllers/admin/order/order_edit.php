    ...
    <div class="page-header">
      ...
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active" aria-current="page">Sửa Đơn</li>
        </ol>
      </nav>
    </div>
    ...
    <div class="row">
      <div class="col-md-12 grid-margin">
        <div class="card card-danhmuc mb-4">
          <h4 class="card-header bg-primary text-white">Sửa Thông Tin</h4>
          <div class="card-body">
           ...
           <form method="post" enctype="multipart/form-data">
              <div class="form-row">
                <div class="pb-2">
                  <a href="/admin/customer/edit/<?=$mitem->cus_id?>">Thông Tin Khách Hàng: <?=$mitem->cus_name?></a>
                </div>
                
                <div class="form-group col-md-12">
                  <label for="cus_id">Khách Hàng</label>
                  <select class="form-control" name="cus_id">
                    <!--Seleted if fit with customer id of this order -->
                    <?php foreach($all_customer as $item):?>
                      <option value="<?=$item->id?>" <?= $item->id == $mitem->cus_id? 'selected': ''?>><?=$item->name.'-'.$item->phone?></option>
                    <?php endforeach;?>
                  </select>
                </div>
                <div class="form-group col-md-12">
                  <label for="shipper_id">NV Giao Hàng</label>
                  <select class="form-control" name="shipper_id" value="<?=$mitem->shipper_id?>">
                    <?php foreach($all_shipper as $item):?>
                      <option value="<?= $item->id.'::'.$item->name.' - '.$item->phone?>" 
                        <?= explode('::', $mitem->shipper_id)[0] == $item->id? 'selected':''?>
              ...
              <?foreach($product_detail as $k => $v):?>
                <div class="form-row col-md-12 product-row">
                  ...
                  <div class="form-group col-md-2">
                    ...
                    <input class="one_price form-control" name="price[]" type="text" readonly value="<?=$v->price?>">
                  </div>
                  <div class="form-group col-md-2">
                    <label for="qty[]">Số Lượng</label>
                    <input class="qty form-control" name="qty[]" type="number" onChange="update_cart($(this))" value="<?=$v->qty?>" >
                  </div>
              ...
              <?php endforeach;?>
                <div class="form-group col-md-12">
                  <div class="col-md-5 float-right">
                    ...
                    <input class="form-control" type="number" name="pre_paid" value="<?=$mitem->pre_paid?>" >
                  </div>
                </div>
